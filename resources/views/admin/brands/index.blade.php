...
<h3 class="fw-bold mb-1">🏷️ Brands</h3>
<p class="mb-4" style="color:#888;">Manage the brands available in your store</p>

<div class="settings-row">
    <form action="{{ route('brands.store') }}" method="POST" class="d-flex align-items-center gap-3 w-100">
        @csrf
        <div class="input-col">
            <input type="text" name="name" class="form-control" placeholder="Brand Name" required>
        </div>
        <button type="submit" class="btn-save-settings">Add Brand</button>
    </form>
</div>
...
<div class="table-responsive mt-4">
    <table class="table table-hover align-middle">
        ...
        <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" class="btn btn-sm btn-danger"
                                onclick="openDeleteModal('delete-form-{{ $brand->id }}', '{{ $brand->name }}')">
                            🗑 Delete
                        </button>

                        <form id="delete-form-{{ $brand->id }}" action="{{ route('brands.destroy', $brand) }}" method="POST" class="d-none">
                            @csrf
                            @method('DELETE')
                        </form>
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="3" class="text-center" style="color:#8b8f9a;">No Brands Found</td>
                </tr>
            @endforelse
        </tbody>
    </table>
...
<div class="delete-modal-overlay" id="deleteModalOverlay">
    <div class="delete-modal-box">
        <h5>Delete Brand?</h5>
        <p id="deleteModalText">Are you sure you want to delete this brand? This cannot be undone.</p>
